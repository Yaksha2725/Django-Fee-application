...
# def retrieve_payment_data(request):
#     if request.method == 'POST':
#         student_id = request.POST.get('student_id')
#         start_date = request.POST.get('start_date')
#         end_date = request.POST.get('end_date')

#         # Query database to retrieve payment data based on criteria
#         payments = Payment.objects.filter(student_id=student_id, date_of_payment__range=[start_date, end_date])

#         # Pass retrieved data to a new template for display
#         return render(request, 'payment_data.html', {'payments': payments})
#     else:
